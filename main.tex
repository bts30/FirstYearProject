...
\\
To this end, consider the simple nonlinear recurrence relation
=======
We will soon see that our experiment exhibits chaotic behaviour for certain sets of parameters. Therefore it can be useful to take a moment to examine a simpler chaotic system which has many of the same qualities our experiment has. To this end, consider the simple non-linear recurrence relation
>>>>>>> origin/master
...
The attractor can be visualized with a graphical algorithm with which the logistic map can also be easily iterated without use of a computer or calculator. First, we plot the functions $f(x)=\lambda x (1-x)$ and $g(x)=x$. Next, we choose an $x_0$. Now, we draw a vertical line from $x_0$ until it intersects $f(x)$. At the intersection we find the value of $x_1$ on the $y$-axis, since all we did was take our $x_0$ and plug it into the function $f(x)=\lambda x (1-x)$. To get $x_2$, we must now do the same we just did with $x_0$, only for $x_1$. To this end we need to find $x_1$ on the $x$-axis instead of the $y$-axis. Therefor, we go horizontally to the graph for $g(x)=x$. The $x$- and $y$-coordinates of the intersection now both mark our value $x_1$, and we can vertically return to our graph of $f(x)$ to obtain $x_2$ on the $y$-axis, which we again convert to the $x$-axis by going to $g(x)$, and so on. Through many iterations and if sweeping for $0<x_0<1$ and $0<\lambda<3$, we see that for $n\to \infty$, $x_n$ tends towards these attractors. 
...
\begin{figure}
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=0.4\linewidth]{Figures/cobweb_x6_l29_iter50}
		\caption{}
		\label{fig:cobwebx6l29iter50}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=0.4\linewidth]{Figures/cobweb_x1_l29_iter50}
		\caption{}
		\label{fig:cobwebx1l29iter50}
	\end{subfigure}
