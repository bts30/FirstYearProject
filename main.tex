...
\usepackage[left=2.5cm,top=2.0cm,bottom=1.5cm, right=2.5cm]{geometry}
%\usepackage{ulem}
...
Within these restrictions, we can grahpically iterate \label{logmap} by 
...
figure \ref{} If we play around with different values for $\lambda$ and $x_0$, 
we quickly see that while !!UNFINISHED!!
...
Seeing the plots of frequency vs amplitude, one might suspect the natural frequency of the system to be close to 20 Hz. To confirm this suspicion we have conducted two experiments. First we  measured to real phase of the system and compared it to the apparent phase. The signal used to control the speaker would give us a reference phase, which we have compared to the measured phase from accelerometer data. A plot of phase shift vs frequency can be seen on figure \ref{phaseshift_plot}.
\begin{figure}[h]
	\centering
	\begin{subfigure}[t]{0.49\textwidth}
...
\end{figure}
Since the loudspeaker can be model as a damped oscillator, we observed the the sigmoid shape of the phase shift resembles the graph. This measurements corroborates our earlier suggestion of the natural frequency being in the middle of the phase shift range suggesting the need for a correction model.

The other experiment conducted was measurement of the acceleration after a sharp collision as seen on figure \ref{ringout}. We then fitted a exponentially deceasing sine function, as that is the acceleration anticipated from a damped oscillator. Also note the frequency $\omega$ found on the plot, which is $\omega = \SI{127.6}{rad/s} = \SI{20.3}{Hz}$. exactly what we expected. This quite firmly proves that loudspeaker indeed can be model as a damped oscillator.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{ballanal.eps}
...
\begin{figure}[h]
	...
	\caption{Period 3 and period 5 orbits.}
\end{figure}
