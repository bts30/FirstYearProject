...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Phase shift between the loudspeaker and the micro controller driving it as a function of frequency. Fit:  $\arctan \left(\frac{2\pi a f}{4\pi^2f^2-b}\right)+2\pi c f$, with $a=-40.6$, $b=1.34 \cdot 10^4$ and $c=1.55 \cdot 10^-2$.}
...
\phi &= \arctan\left( \frac{b\omega}{m\omega^2-k} \right)=\arctan\left(\frac{2\pi b f}{4\pi^2 m f^2-k}\right)
\end{align*}
...
\begin{equation}
    \phi = \arctan\left(\frac{2\pi b f}{4\pi^2 m f^2-k}\right)+\phi_{latent} =\arctan\left(\frac{2\pi b f}{4\pi^2 m f^2-k}\right) + 2\pi f t
    \label{pshifteq}
\end{equation}
We use this equation to model the phaseshift in our loudspeaker, since it seems reasonable to suggest that there will be some delay in the signal on its way to the loudspeaker.
...
\begin{figure}[h]
	...
	\begin{subfigure}{0.45\textwidth}
	    ...
	    \caption{Odd numbered orbits in experimental data.}
	\end{subfigure}\hfill
	\begin{subfigure}{0.45\textwidth}
		\centering
